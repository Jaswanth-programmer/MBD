const memories=[
 ...
 ["mom3.jpg","నీ నవ్వు నాకు ఎప్పుడూ ఇష్టం"],
...
function hearts(n=80){
 ...
 for(let i=0;i<n;i++){
  ...
  h.style.setProperty("--spin",((Math.random()-.5)*100)+"deg");
  h.style.animationDuration=(3.2+Math.random()*3.5)+"s";
...
function confetti(){
 ...
 const pieces=Array.from({length:220},()=>({
  ...
  g:.3,s:2+Math.random()*6,r:Math.random()*6,l:120+Math.random()*80
 }));
 let f=0;
 function draw(){
  ctx.clearRect(0,0,c.width,c.height);
  pieces.forEach(p=>{
   p.x+=p.vx;p.y+=p.vy;p.vy+=p.g;p.r+=.12;p.l--;
   ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.r);
   ctx.fillStyle=`hsl(${330+Math.random()*45},75%,72%)`;
...
document.getElementById("reveal").onclick=()=>{
 hearts(130);
 confetti();
 setTimeout(()=>document.getElementById("story").scrollIntoView({behavior:"smooth"}),700);
};
